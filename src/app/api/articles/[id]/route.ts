...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const extra = touchedStage ? ", stage_moved_at = date('now')" : "";

  run(
    `UPDATE articles SET ${fields.map((f) => `${f} = ?`).join(", ")}${extra}, updated_at = datetime('now') WHERE id = ?`,
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  const { id } = await ctx.params;
  run("UPDATE articles SET archived = 1 WHERE id = ?", Number(id));
